...
abstract class sly_StructureTest extends sly_BaseTest {
	...
	protected function tearDown() {
		...
		$result = $this->pdo->query('SELECT count(id) FROM sly_article WHERE re_id NOT IN (SELECT id FROM sly_article WHERE 1) AND re_id <> 0');
		$count  = (int) $result->fetch(PDO::FETCH_COLUMN);
		$this->assertEquals(0, $count, 're_id may never point to a non-existing category');

		// sanity check: positions may never be negative
		$result = $this->pdo->query('SELECT count(id) FROM sly_article WHERE pos < 0 OR catpos < 0');
		$count  = (int) $result->fetch(PDO::FETCH_COLUMN);
		$this->assertEquals(0, $count, 'pos and catpos may never be negative');
	}
